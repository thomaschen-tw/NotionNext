    ...
    for (let index = 0; index < siteIds.length; index++) {
      ...
      if (index === 0 || locale === prefix) {
        try {
          ...
          // 捕获 Notion API 错误，避免未处理的 rejection
          console.error(`[getGlobalData] 获取 Notion 数据失败 (pageId: ${id}, from: ${from}):`, error.message || error)
          // 继续使用空数据，不抛出错误
        }
      }
    }
  } catch (error) {
    console.error('[getGlobalData] 异常:', error.message || error)
  }
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  try {
    return await getOrSetDataWithCache(
      ...
      async (pageId, from) => {
        try {
          ...
          if (!pageRecordMap) {
            console.warn(`[getSiteDataByPageId] 无法获取页面数据 (pageId: ${pageId})`)
            return EmptyData(pageId)
          }
          return convertNotionToSiteData(pageId, from, deepClone(pageRecordMap))
        } catch (error) {
          console.error(`[getSiteDataByPageId] 获取页面数据失败 (pageId: ${pageId}):`, error.message || error)
          // 返回空数据而不是抛出错误
          return EmptyData(pageId)
        }
      },
    ...
    console.error(`[getSiteDataByPageId] 缓存或数据处理失败 (pageId: ${pageId}):`, error.message || error)
    // 返回空数据而不是抛出错误
    return EmptyData(pageId)
  }
}
